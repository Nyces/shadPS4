...
bool Rasterizer::BindResources(const Pipeline* pipeline) {
    ...
    if (output_upscaled && liverpool->regs.IsClipDisabled()) {
        // Clip-disabled passes pin the Vulkan viewport to the hardware window and
        // convert positions to window coordinates in the shader through push data
        // (see ConvertPositionToClipSpace), so the stretch cannot go through the
        // viewport and has to be carried by the conversion itself. Scaling the
        // offsets together with the scales is a uniform affine map: it stretches
        // the rectangle and its tessellation-interpolated fourth corner without
        // distorting it, and keeps screen-space math reading the same constants
        // consistent with the enlarged extent.
